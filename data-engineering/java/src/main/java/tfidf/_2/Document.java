...
/**
 * Class that represents a text document Keeps track of the number of times a word appears in the document, it's term
 * frequency, and eventually, its inverse document frequency (if used with TfIdf) for finding important keywords in the
 * document
 * 
 * 
 * @author Barkin Aygun
...
public class Document {
	 ...
	 * 
	 * @param br
	 *            Reader that loaded the text file already, used to read lines from large documents
	 * @param parent
	 *            the TfIdf class calling this ctor
	 */
	public Document(BufferedReader br, Tfidf parent) {
		...
		// debug
		System.out.println();
		TreeMap<String, Double[]> copiedWords = words;
		int i = 1;
		for (Iterator<String> it = copiedWords.keySet().iterator(); it.hasNext();) {
			String str = it.next();
			System.out.println(i++ + " => " + str + "\t" + copiedWords.get(str)[0]);
		}
	...
	public Document(BufferedReader br, String docName, Tfidf parent) {
		String line;
		String word;
		StringTokenizer tokens;
		sumof_n_kj = 0;
			...
			while (line != null) {
				...
				while (tokens.hasMoreTokens()) {
					...
					if (word.length() < 2)
						continue;
					if (words.get(word) == null) {
						tempdata = new Double[] { 1.0, 0.0, 0.0 }; // word_count, tf-value, idf-value
						words.put(word, tempdata);
					} else {
						tempdata = words.get(word);
						tempdata[0]++;
		...
		for (Iterator<String> it = words.keySet().iterator(); it.hasNext();) {
			...
			words.put(word, tempdata);
			parent.addWordOccurence(word);
			// parent.addWordOccurence2(word, tempdata);
		}

		// debug
		System.out.println("\nDoc Name => " + docName + "\t" + "Word count number => " + sumof_n_kj);
		System.out.println("-------------------------------------------------------------------------");
		TreeMap<String, Double[]> copiedWords = words;
		int i = 1;
		for (Iterator<String> it = copiedWords.keySet().iterator(); it.hasNext();) {
			String str = it.next();
			System.out.println(i++ + ": word => " + str + "\t wordcnt => " + copiedWords.get(str)[0]+ "\t tf => " + copiedWords.get(str)[1]);
		}
	}
